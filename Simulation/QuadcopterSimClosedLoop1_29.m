...
initialConditions = zeros(17,1);
animateFlag = false; %set to true if you want to see the full animation, set to false if you only want plots
animateStepSize = 300; %bigger means you will see more fewer little quadcopter depictions in the animation/trajectory plot
% initialConditions(8)=-0.2;
% initialConditions(7)=0.1;
initialConditions(9) = deg2rad(0);


%% controller parameters

%reference inputs
in.zDes = -1; %NOTE: This should always be negative
in.xDes = 2; 
in.yDes = 1;
in.yawDes = deg2rad(0);
...
in.zD = 4;
in.zI = 0.5;
in.xyP = 0.005; %using this for inner loop lateral controller
in.xyD = 0.03; 
...
function [Sdot,in] = quadcopterDynamics(t,S,in)
    ...
    F = in.kThrust*omega.^2; %thrust from ith motor
    ...
    xyzDDot = -(rotationMatrix(roll,pitch,yaw)*[0; 0;sum(F)])/in.m + [0; 0; 9.81];
    ...
    Sdot = [xDot yDot zDot xyzDDot' rollRate pitchRate yawRate ...
        rollDDot pitchDDot yawDDot omegaDot' zError]';    
...
function [V,zError,yawError,pitchError,rollError,in] = hoverController(t,S,in)
    ...
    %If you want to take out the yaw adjustments
    %XErrorRel = XError;
    %YErrorRel = YError;
    %XDerivRel = -xDot;
    %YDerivRel = -yDot;
